...
# Wallpaper directory
wpDir="$HOME/Pictures/walls/all/"

# Wallpaper list path
wpList=$HOME/Pictures/walls/all/.wallpaper-list
...
cmdTile="eval  DISPLAY=:0 ogurictl output '*' --image "
...
cmdScale="eval DISPLAY=:0 ogurictl output '*' --image "
...
cmdCenter="eval DISPLAY=:0  ogurictl output  '*' --image "
	...
	if [[ (-w "$wpList") ]]; then
		...
		if [[ "$dontSkip" == "false" ]]; then
			for dir in "${wpSkip[@]}"; do
				grep -Ev "$dir" "$wpList" >~/.wallpapers-tmpr
				mv ~/.wallpapers-tmpr "$wpList"
			done
		fi
		# Output result
...
getImage() {
	...
	if [[ (-s "$wpList" && -f "$wpList") && -r "$wpList" ]]; then
...
setWallpaper() {
	...
	if [[ ("$scaleLowerRes" == "yes") && ("$imgAspect" != "1.0" && ("$imgWidth" -lt "$resWidth" || "$imgHeight" -lt "$resHeight")) ]]; then
		setScaled "$wpPath"
	else
		case $imgAspect in
		1.0)
...
checkConfig() {
	...
	if [[ !( ( -n "$wpDir" ) && ( -n "$wpList" ) && ( -n "$resWidth" ) && ( -n "$resHeight" ) && ( -n "$scaleLowerRes" ) && ( -n "$cmdTile" ) && ( -n "$cmdScale" ) && ( -n "$cmdCenter" ) ) ]]; then
	...
	if [[ !( "$scaleLowerRes" == "yes" || "$scaleLowerRes"  == "no" ) ]]; then
